...
def get_args_parser(add_help=True):
    parser = argparse.ArgumentParser(
        description="ev-YOLOv6 PyTorch Training", add_help=add_help
    )
    parser.add_argument(
        "--file",
        type=str,
        required=True,
        default="/data/storage/datasets/gen1/h5_single_files",
    )
    parser.add_argument("--training", action="store_true")
    parser.add_argument("--num_events", type=int, default=50000)
    parser.add_argument("--time_window", type=int, default=300000)
    parser.add_argument(
    ...
    parser.add_argument("--augment", action="store_true", help="augment or not")
    parser.add_argument("--rect", action="store_true", help="rectangle or not")
    parser.add_argument(
        "--representation", default="SPF", type=str, help="Event representation"
    )
    parser.add_argument(
        "--dataset", default="gen1", type=str, help="Dataset name (gen1, gen4)"
    )
    parser.add_argument(
        "--fuse_ab",
        action="store_true",
    ...
    parser.add_argument(
        "--testing", action="store_true", help="whether to evaluate on the test set"
    )
    parser.add_argument(
        "--wandb_name", default="default_name", type=str, help="Wandb AI name"
    )
    parser.add_argument(
        "--wandb_dir",
        default="/data/nzubic/Projects/event_representation_study/",
        type=str,
        help="Wandb AI directory for saving logs",
    )
...
if __name__ == "__main__":
    ...
    if not args.testing:
        wandb.init(
            project="event_representation_study",
