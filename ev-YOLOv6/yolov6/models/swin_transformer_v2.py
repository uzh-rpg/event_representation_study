...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask=None):
        ...
        logit_scale = torch.clamp(
            self.logit_scale, max=torch.log(torch.tensor(1.0 / 0.01)).cuda()
        ).exp()
...
class SwinTransformer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(len(outs)):
            outs[i] = nn.LayerNorm(outs[i].shape[-1]).cuda()(outs[i])  # B L C
            wh = int(torch.sqrt(torch.Tensor([outs[i].shape[1]])).item())
            outs[i] = (
                outs[i]
                .view(-1, wh, wh, outs[i].shape[-1])
                .permute(0, 3, 1, 2)
                .contiguous()
            )
            outs[i] = nn.AdaptiveAvgPool3d(adaptive_average_pooling_sizes[i])(outs[i])
...
def swin_transformerv2(pretrained=False, channels=3, **kwargs):
