...
class Evaler:
    ...
    def predict_model(self, model, dataloader, task):
        ...
        target_num = random.sample(range(0, 318), 1)[0]
        ...
        for i, (imgs, targets, paths, shapes) in enumerate(pbar):
            ...
            if not self.check_layer_exists(model, "quantization_layer"):
                imgs /= 255
            ...
            # for wandb visualization, maximum images to show: 8
            if i == target_num:
                # vis_num = min(len(imgs), 8)
                # vis_outputs = outputs[:vis_num]
                # vis_paths = paths[:vis_num]
                try:
                    indexes = operator.itemgetter(
                        *random.sample(range(0, len(imgs)), 8)
                    )
                    vis_outputs = indexes(outputs)
                    vis_paths = indexes(paths)
                except:
                    pass
    ...
    def eval_model(self, pred_results, model, dataloader, task):
        ...
        if task != "speed" and len(pred_results):
            ...
            if self.verbose:
                ...
                label_count_dicts = [
                    copy.deepcopy(label_count_dict) for _ in range(model.nc)
                ]
    ...
    def convert_to_coco_format(self, outputs, imgs, paths, shapes, ids):
        ...
        for i, pred in enumerate(outputs):
            ...
            image_id = int(path.stem) if self.is_coco else str(path)
